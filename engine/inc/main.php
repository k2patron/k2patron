...
if ( $config['allow_cache'] == "yes" AND !$config['cache_type'] ) {
...
if( $stats_arr['count_c_app'] ) {
	...
	$stats_arr['count_c_app'] = $stats_arr['count_c_app'] . " [ <a href=\"?mod=cmoderation\">{$lang['stat_cmod_link']}</a> ]";

}

if( $stats_arr['approve'] and $user_group[$member_id['user_group']]['allow_all_edit'] ) {
	
	$stats_arr['approve'] = $stats_arr['approve'] . " [ <a href=\"?mod=editnews&action=list&news_status=2\">{$lang['stat_medit_link']}</a> ]";
...
<table width="100%">
    <tr>
        <td width="265" style="padding:2px;">{$lang['site_status']}</td>
        <td>{$offline}</td>
    </tr>
    <tr>
        <td style="padding:2px;">{$lang['stat_allnews']}</td>
    ...
    <tr>
        ...
        <td><font color="red">{$stats_arr['stats_banned']}</font></td>
    </tr>
    <tr>
...
HTML;
	...
	if ($stats_arr['count_subscribe']) echo "&nbsp;<input id=\"clearsubscribe\" name=\"clearsubscribe\" class=\"btn btn-danger\" type=\"button\" value=\"{$lang['btn_clearsubscribe']}\">";
...
HTML;
	...
	if(!is_writable(ENGINE_DIR . '/data/config.php')) {
		...
		$fail = "<br /><div class=\"ui-state-error ui-corner-all\" style=\"padding:10px;\">{$lang['stat_system']}</div>";
		...
		echo "<br /><table width=\"100%\"align=center><tr><td style='padding:3px; border:1px dashed rgb(190,190,190); background-color:lightyellow;'>
         <div id=\"dle-activation\">$lang[trial_info]<br /><br /><b>$lang[trial_key]</b><span style=\"padding-left:7px;\"><input class=\"edit bk\" type=\"text\" size=\"45\" name=\"sitekey\" id=\"sitekey\"> <input class=\"btn btn-success btn-mini\" type=\"button\" onClick=\"dle_activation( 'key' ); return false;\" value=\"$lang[trial_act]\"></span><div id=\"result_info\" style=\"color:red;\"><br /><span class=\"navigation\">{$lang['key_format']} <b>XXXXX-XXXXX-XXXXX-XXXXX-XXXXX</b></span></div></div>{$fail}
	...
	if (!defined( 'SECURE_AUTH_KEY' ) OR strlen(SECURE_AUTH_KEY) < 20 ) {
		echo "<br><table width=\"100%\" align=center><tr><td>
         <div class=\"ui-state-error ui-corner-all\" style=\"padding:10px;\">{$lang['stat_sec_auth']}</div>
         </td></tr><tr><td>&nbsp;</td></tr></table>";
	}
