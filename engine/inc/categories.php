...
<table width="100%">
    ...
    <tr>
        <td style="padding:4px;">{$lang['opt_sys_sub']}</td>
        <td>{$row['show_sub']}</td>
    </tr>
    <tr>
	...
	function DisplayCategories($parentid = 0, $sublevelmarker = false) {
		...
		if( count( $cat_info ) ) {
			...
			if( count( $root_category ) ) {
				...
				if( $sublevelmarker ) return "<ol>".$cat_item."</ol>"; else return $cat_item;
...
HTML;
    ...
    <tr>
        <td colspan="7" style="padding:5px;"><button id="catsort" class="btn btn-primary">{$lang['cat_posi']}</button></td>
    </tr>
     </table>
</td>
        <td background="engine/skins/images/tl_rb.gif"><img src="engine/skins/images/tl_rb.gif" width="6" height="1" border="0"></td>
