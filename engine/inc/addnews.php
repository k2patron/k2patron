...
<img src="engine/skins/images/img.gif"  align="absmiddle" id="f_trigger_c" style="cursor: pointer; border: 0" title="{$lang['edit_ecal']}"/>&nbsp;<input type="checkbox" name="allow_date" value="yes" checked>&nbsp;{$lang['edit_jdate']}<a href="#" class="hintanchor" onMouseover="showhint('{$lang[hint_calendar]}', this, event, '320px')">[?]</a>, {$lang['edit_eau']} {$author_info}
<script type="text/javascript">
    Calendar.setup({
...
HTML;
    ...
    <tr>
        ...
        <td>{$bb_code}<textarea rows="16" style="width:98%; padding:0px;" onfocus="setFieldName(this.name)" name="short_story" id="short_story" class="bk"></textarea>
	</td></tr>
HTML;
	}
    ...
    <td>{$bb_panel}<textarea rows="19" onfocus="setFieldName(this.name)" name="full_story" id="full_story" style="width:98%;" class="bk"></textarea>
	</td></tr>
HTML;
	}
	...
	if( !$config['allow_admin_wysiwyg'] ) $fix_br = "<input type=\"checkbox\" name=\"allow_br\" value=\"1\" checked>&nbsp;{$lang['allow_br']}";
	else $fix_br = "";
...
<div style="padding-left:150px;padding-top:5px;padding-bottom:5px;">
	<input type="submit" class="btn btn-success" value="{$lang['news_add']}" style="width:100px;">&nbsp;
...
elseif( $action == "doaddnews" ) {
	...
	$db->query( "UPDATE " . PREFIX . "_images SET news_id='{$row}' where author = '{$author}' AND news_id = '0'" );
	$db->query( "UPDATE " . PREFIX . "_files SET news_id='{$row}' where author = '{$author}' AND news_id = '0'" );
